...
@Configuration
@MapperScan(basePackages = "com.xq.hdb.mapper.db4", sqlSessionFactoryRef = "db4SqlSessionFactory")
public class Db4DataSourceConfig {
    @Bean("db4DataSource")
    @ConfigurationProperties(prefix = "spring.datasource.db4") //读取application.yml中的配置参数映射成为一个对象
    ...
    public SqlSessionTemplate db3SqlSessionTemplate(@Qualifier("db4SqlSessionFactory") SqlSessionFactory sqlSessionFactory) {
        return new SqlSessionTemplate(sqlSessionFactory);
    }

    @Bean("db4MybatisPlusInterceptor")
